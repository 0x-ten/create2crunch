...
pub struct Config {
    ...
    pub desired_prefix: Option<Vec<u8>>,
    pub desired_suffix: Option<Vec<u8>>,
}
...
impl Config {
    pub fn new(mut args: std::env::Args) -> Result<Self, &'static str> {
        ...
        // New parameters for desired prefix and suffix (optional)
        let desired_prefix_string = match args.next() {
            Some(arg) => arg,
            None => String::from(""), // Empty string means no specific prefix
        };
        let desired_suffix_string = match args.next() {
            Some(arg) => arg,
            None => String::from(""), // Empty string means no specific suffix
        };
        ...
        // Parse the desired prefix and suffix (if provided)
        let desired_prefix = if desired_prefix_string.is_empty() {
            None
        } else {
            // Try to parse as hex first
            if let Ok(prefix_bytes) = hex::decode(&desired_prefix_string) {
                Some(prefix_bytes)
            } else {
                // If not hex, use the string bytes directly
                Some(desired_prefix_string.as_bytes().to_vec())
            }
        };
        let desired_suffix = if desired_suffix_string.is_empty() {
            None
        } else {
            // Try to parse as hex first
            if let Ok(suffix_bytes) = hex::decode(&desired_suffix_string) {
                Some(suffix_bytes)
            } else {
                // If not hex, use the string bytes directly
                Some(desired_suffix_string.as_bytes().to_vec())
            }
        };
    ...
    loop {
        ...
        (0..MAX_INCREMENTER)
            ...
            .for_each(|salt| {
                ...
                // Check for desired prefix and suffix if specified
                let mut matches_prefix = true;
                let mut matches_suffix = true;
                let mut match_info = String::new();
                
                // Convert address to hex string without 0x prefix for pattern matching
                let address_hex = hex::encode(address);
                
                // Check prefix if specified
                if let Some(prefix) = &config.desired_prefix {
                    // Convert prefix to hex string for comparison if it's not already hex
                    let prefix_hex = if prefix.iter().all(|&b| (b as char).is_ascii_hexdigit()) {
                        // If prefix is already hex bytes, just convert to string
                    ...
                    matches_prefix = address_hex.starts_with(&prefix_hex);
                    
                    if matches_prefix {
                        match_info.push_str(&format!("PREFIX({})", prefix_hex));
                    }
                }
                
                // Check suffix if specified
                if let Some(suffix) = &config.desired_suffix {
                    // Convert suffix to hex string for comparison if it's not already hex
                    let suffix_hex = if suffix.iter().all(|&b| (b as char).is_ascii_hexdigit()) {
                        // If suffix is already hex bytes, just convert to string
                    ...
                    // Check if address ends with the desired suffix
                    matches_suffix = address_hex.ends_with(&suffix_hex);
                    
                    if matches_suffix {
                        if !match_info.is_empty() {
                            match_info.push_str(" & ");
                        }
                        match_info.push_str(&format!("SUFFIX({})", suffix_hex));
                    }
                }
                
                // If we have a match for both prefix and suffix (if specified), we'll continue processing
                let has_pattern_match = (config.desired_prefix.is_none() || matches_prefix) && 
                                        (config.desired_suffix.is_none() || matches_suffix);
                
                // If we have at least one pattern specified and we matched it
                let has_custom_pattern = (config.desired_prefix.is_some() || config.desired_suffix.is_some()) && has_pattern_match;
                
                if has_custom_pattern {
                    // For pattern matches, we'll use a special reward key
                    // We'll use a high value to ensure it's considered valuable
                    let key = 400; // Special key for pattern matches
                    let reward_amount = Some("PATTERN_MATCH");
                    ...
                    // display the salt and the address with pattern indicator
                    let output = format!(
                        "{full_salt} => {address} => {}",
                        match_info
                    );
                ...
                // If no pattern is specified or if we didn't match the pattern,
                // fall back to the original zero-counting logic
                if !has_custom_pattern {
